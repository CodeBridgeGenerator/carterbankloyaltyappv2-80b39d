...
const FcmMessagePage = (props) => {
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("fcmMessages").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
