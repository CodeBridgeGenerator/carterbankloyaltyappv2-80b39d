...
const DocumentationDetailsPage = (props) => {
  ...
  useEffect(() => {
    ...
    client
      ...
      .catch((error) => {
        ...
        props.alert({
          ...
          type: "error",
          message: error.message || "Failed get Documentation Details",
        });
      });
  }, [
    showFakerDialog,
    ...
    showCreateDialog,
    showGenerateDialog,
  ]); // Add showGenerateDialog to refresh
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("documentationDetails").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="documentationDetails-add-button"/> */}
      ...
      <AreYouSureDialog
        ...
        onYes={() => deleteRow()}
      />
      <DocumentationDetailsEditDialogComponent
