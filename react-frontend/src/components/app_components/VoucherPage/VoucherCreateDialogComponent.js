...
const VoucherCreateDialogComponent = (props) => {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      const eagerResult = await client.service("voucher").find({
        query: {
          $limit: 10000,
          _id: { $in: [result._id] },
          $populate: [
      ...
      props.alert({
        type: "success",
        title: "Create info",
        message: "Info Voucher updated successfully",
      });
      ...
      props.alert({
        type: "error",
        title: "Create",
        message: "Failed to create in Voucher",
      });
    }
    setLoading(false);
  };
  ...
  const categoryIdOptions = categoryId.map((elem) => ({
    name: elem.name,
    value: elem.value,
  }));
      ...
      >
        <div className="col-12 md:col-6 field">
          <span className="align-items-center">
            ...
            <Dropdown
              id="categoryId"
              value={_entity?.categoryId?._id}
              optionLabel="name"
              optionValue="value"
              options={categoryIdOptions}
              onChange={(e) => setValByKey("categoryId", { _id: e.value })}
            />
          </span>
