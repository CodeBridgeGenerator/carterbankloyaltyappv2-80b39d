...
const CartItemHistoryPage = (props) => {
  ...
  const filename = "cartItemHistory";
  ...
  useEffect(() => {
    ...
    client
      ...
      .find({
        query: {
          $limit: 10000,
          user: urlParams.singleUserId,
          voucher: urlParams.singleVoucherId,
          $populate: [
            {
              path: "createdBy",
              service: "users",
      ...
      .catch((error) => {
        ...
        props.alert({
          ...
          type: "error",
          message: error.message || "Failed get CartItemHistory",
        });
      });
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("cartItemHistory").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
  ...
  const onRowClick = ({ data }) => {
    navigate(`/app/cartItemHistory/${data._id}`);
  };
  ...
  return (
    <div className="mt-5">
        ...
        <div className="col-6 flex justify-content-end">
          <>
            ...
            <Button
              ...
              role="cartItemHistory-add-button"
            />
          </>
        </div>
      </div>
      <div className="grid align-items-center">
        <div className="col-12" role="cartItemHistory-datatable">
          <CartItemHistoryDatatable
            items={data}
            fields={fields}
      ...
      <AreYouSureDialog
        ...
        onYes={() => deleteRow()}
      />
      <CartItemHistoryEditDialogComponent
