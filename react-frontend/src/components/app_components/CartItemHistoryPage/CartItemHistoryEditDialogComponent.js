...
const CartItemHistoryEditDialogComponent = (props) => {
    ...
    client
      ...
      .find({
        query: {
          $limit: 10000,
          $sort: { createdAt: -1 },
          _id: urlParams.singleUserId,
        },
      })
      .then((res) => {
        setUser(
          res.data.map((e) => {
            return { name: e["username"], value: e._id };
          }),
        );
      })
      .catch((error) => {
    ...
    client
      ...
      .catch((error) => {
        ...
        props.alert({
          title: "Voucher",
          type: "error",
          message: error.message || "Failed get voucher",
        });
      });
  }, []);
  ...
  const onSave = async () => {
    ...
    try {
      ...
      const eagerResult = await client.service("cartItemHistory").find({
        query: {
          $limit: 10000,
          _id: { $in: [_entity._id] },
          $populate: [
  ...
  const voucherOptions = voucher.map((elem) => ({
    name: elem.name,
    value: elem.value,
  }));
      ...
      >
        ...
        <div className="col-12 md:col-6 field">
          <span className="align-items-center">
            ...
            <Dropdown
              id="voucher"
              value={_entity?.voucher?._id}
              optionLabel="name"
              optionValue="value"
              options={voucherOptions}
              onChange={(e) => setValByKey("voucher", { _id: e.value })}
            />
          </span>
          <small className="p-error">
        ...
        <div className="col-12 md:col-6 field">
          <span className="align-items-center">
            ...
            <Calendar
              ...
              dateFormat="dd/mm/yy"
              onChange={(e) => setValByKey("timestamp", new Date(e.value))}
              showIcon
              showButtonBar
            />
          </span>
